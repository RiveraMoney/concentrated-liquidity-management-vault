...
contract CakeLpStakingV2Test is Test {
    ...
    address _stake = 0xBd2d94d09AbaDc5084f278bd639Fbc9Af6A6bea0;//linea()  
    address _chef = address(0);   //Address of the pancake master chef v2 contract on BSC mainnet
    address _router = 0x6aa397CAB00a2A40025Dbf839a83f16D5EC7c1eB; //linea
    address _nonFungiblePositionManager = 0xBa2e5317CC21CF591d3908F703a855547cDc849f;///linea
    address _weth = 0x2C1b868d6596a18e32E61B901E4060C872647b6C;//linea   //Adress of the CAKE ERC20 token on mainnet
    address _usdc = 0xf56dc6695cF1f5c364eDEbC7Dc7077ac9B586068;//linea
    bool hasFarm = false;

    //cakepool params
    bool _isTokenZeroDeposit = false;
    int24 _tickLower =-101020;
    int24 _tickUpper =-100300;
    address _cakeReward = address(0);
    //libraries
    address _tickMathLib = 0xA9C3e24d94ef6003fB6064D3a7cdd40F87bA44de;//linea
    address _sqrtPriceMathLib = 0x25ADF247aC836D35be924f4b701A0787A30d46a9;//linea
    address _liquidityMathLib = 0xe931672196AB22B0c161b402B516f9eC33bD684c;//linea
    address _safeCastLib = 0xA7B88e482d3C9d17A1b83bc3FbeB4DF72cB20478;//linea
    address _liquidityAmountsLib = 0x74C5E75798b33D38abeE64f7EC63698B7e0a10f1;//linea
    address _fullMathLib = 0x672058B73396C78556fdddEc090202f066B98D71;//linea
    ...
    address[] _rewardToLp0AddressPath = [_cakeReward, _usdc];
    uint24[] _rewardToLp0FeePath = [2500];
    address[] _rewardToLp1AddressPath = [_cakeReward, _weth];
    uint24[] _rewardToLp1FeePath = [2500];

    ///@dev Vault Params
    ///@notice Can be configured according to preference
    string rivTokenName = "RivWETH-USDC";
    string rivTokenSymbol = "rivWETH-USDC";
    ...
    address _whale = 0xc0dEC722b431c02a0787F349587B783A0f2F3281;        //35 Mil whale 35e24
    uint256 _maxUserBal = 15e9;
    ...
    function setUp() public {
        ...
        vault = new RiveraAutoCompoundingVaultV2Public(_usdc, rivTokenName, rivTokenSymbol, stratUpdateDelay, vaultTvlCap);
        ...
        //log balance of _whale
        emit log_named_uint("Whale balance", IERC20(_usdc).balanceOf(_whale));
        IERC20(_usdc).transfer(_user1, _maxUserBal);
        IERC20(_usdc).transfer(_user2, _maxUserBal);
        vm.stopPrank();
    }

    function test_GetDepositToken() public {
        address depositTokenAddress = strategy.depositToken();
        assertEq(depositTokenAddress, _usdc);
    }

    ///@notice tests for deposit function

    function test_DepositWhenNotPausedAndCalledByVaultForFirstTime() public {
        uint256 poolTvl = IERC20(_usdc).balanceOf(_stake) + DexV3Calculations.convertAmount0ToAmount1(IERC20(_weth).balanceOf(_stake), _stake, _fullMathLib);
        ...
        vm.prank(_user1);
        IERC20(_usdc).transfer(address(strategy), depositAmount);
        ...
        uint256 usdtBal = IERC20(_usdc).balanceOf(address(strategy));
        ...
        uint256 wbnbBal = IERC20(_weth).balanceOf(address(strategy));
    ...
    //     uint256 _wethReceived = IV3SwapRouter(_router).exactInputSingle(
    ...
    function test_WithdrawWhenCalledByVault() public {
        ...
        // uint256 vaultDenominaionbal = IERC20(_usdc).balanceOf(address(vault));
        ...
        // vaultDenominaionbal = IERC20(_usdc).balanceOf(address(vault));
    ...
    // //     uint256 usdtBalBef = IERC20(_usdc).balanceOf(address(strategy));
    // //     uint256 wbnbBalBef = IERC20(_weth).balanceOf(address(strategy));
    ...
    // //     // if(IERC20(_usdc).balanceOf(address(strategy)) > usdtBalBef) {
    // //     //     assertLe(IERC20(_usdc).balanceOf(address(strategy)) - usdtBalBef, 5 * _convertRewardToToken0(rewardsAvblBef) / 1000);
    // //     // } else {
    // //     //     assertLe(IERC20(_usdc).balanceOf(address(strategy)), 5 * _convertRewardToToken0(rewardsAvblBef) / 1000);
    // //     // }
    // //     // if (IERC20(_weth).balanceOf(address(strategy)) > wbnbBalBef) {
    // //     //     assertLe(IERC20(_weth).balanceOf(address(strategy)) - wbnbBalBef, 5 * _convertRewardToToken1(rewardsAvblBef) / 1000);
    // //     // } else {
    // //     //     assertLe(IERC20(_weth).balanceOf(address(strategy)), 5 * _convertRewardToToken1(rewardsAvblBef) / 1000);
    // //     // }
