...
contract RiveraAutoCompoundingVaultV2PreviousTest is Test {
    ...
    function test_Balance() public {
        ...
        totalBalance = vault.totalAssets();
    ...
    function test_WithdrawWhenCalledByOwner() public {
        ...
        vm.prank(_user);
        vault.withdraw(1e18, _user, _user);

        uint256 userShareAfter = vault.balanceOf(_user);
        assertGt(userShareAfter, 1e18);
        uint256 userStakeBalanceAfter = IERC20(_stake).balanceOf(_user);
        assertEq(userStakeBalanceAfter - userStakeBalanceBefore, 1e18);
        uint256 vaultBalanceAfter = vault.totalAssets();
        assertEq(vaultBalanceBefore - vaultBalanceAfter, 1e18);
